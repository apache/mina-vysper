...
public class ComponentStanzaProcessorTestCase {
    ...
    private ComponentStanzaProcessor processor = new ComponentStanzaProcessor(serverRuntimeContext);
    ...
    public void processSuccessfulWithResponse() throws ProtocolException, DeliveryException {
        Mockito.when(handler.execute(stanza, serverRuntimeContext, false, sessionContext, sessionStateHolder)).thenReturn(container);
    ...
    public void handlerThrowsException() throws ProtocolException, DeliveryException {
        Mockito.when(handler.execute(stanza, serverRuntimeContext, false, sessionContext, sessionStateHolder)).thenThrow(new ProtocolException());
    ...
    public void processThenFailRelaying() throws ProtocolException, DeliveryException {
        Mockito.when(handler.execute(stanza, serverRuntimeContext, false, sessionContext, sessionStateHolder)).thenReturn(container);
        Mockito.doThrow(new DeliveryException()).when(stanzaRelay).relay(FROM, responseStanza, IgnoreFailureStrategy.IGNORE_FAILURE_STRATEGY);
