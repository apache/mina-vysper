 ...
 */
public class MUCMessageStanzaBrokerProvider implements MAMStanzaBrokerProvider {
    @Override
    public boolean supports(Stanza processedStanza, ServerRuntimeContext serverRuntimeContext) {
        if (!MessageStanza.isOfType(processedStanza)) {
            return false;
        }

        Entity to = processedStanza.getTo();
        if (to == null) {
            return false;
        }

        return !serverRuntimeContext.getServerEntity().getDomain().equals(to.getDomain());
    }
